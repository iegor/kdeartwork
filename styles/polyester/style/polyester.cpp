    ...
    flatMode(false)
{
    ...
    settings.beginGroup("/polyesterstyle/Settings");
    _useLowerCaseText = settings.readBoolEntry("/useLowerCaseText", false);
    ...
    _scrollBarExtent = settings.readNumEntry("/scrollBarSize", 16);
    if(_scrollBarExtent < 8 || _scrollBarExtent > 48)
      _scrollBarExtent = 16;
    _animateProgressBar = settings.readBoolEntry("/animateProgressBar", false);
    _animateButton = settings.readBoolEntry("/animateButton", false);
    ...
    _animateButtonToDark = settings.readBoolEntry("/animateButtonToDark", false);
    _buttonStyle = settings.readEntry("/buttonStyle", "glass");
    _lightBorder= settings.readBoolEntry("/lightBorder", true);
    _coloredScrollBar = settings.readBoolEntry("/coloredScrollBar", true);
    _centeredTabBar = settings.readBoolEntry("/centeredTabBar", false);
    _highLightTab = settings.readBoolEntry("/highLightTab", true);
    _statusBarFrame = settings.readBoolEntry("/statusBarFrame", true);
    _colorizeSortedHeader = settings.readBoolEntry("/colorizeSortedHeader", true);
    _menuItemSpacing = settings.readNumEntry("/menuItemSpacing", 8);
    if( _menuItemSpacing > 12 || _menuItemSpacing < 0 )
      _menuItemSpacing = 8;
    _buttonMenuItem = settings.readBoolEntry("/buttonMenuItem", true);
    _menuBarEmphasis = settings.readBoolEntry("/menuBarEmphasis", false);
    _menuBarEmphasisBorder = settings.readBoolEntry("/menuBarEmphasisBorder", true);
    if( _menuBarEmphasis )
        _customMenuBarEmphasisColor = settings.readBoolEntry("/customMenuBarEmphasisColor", false);
    else
        _customMenuBarEmphasisColor = false;
    _menuBarEmphasisColor = settings.readEntry("/menuBarEmphasisColor", "black");
    _menuStripe = settings.readBoolEntry("/menuStripe", true);
    _shadowedMenuBarText = settings.readBoolEntry("/shadowedMenuBarText", true);
    _shadowedButtonsText = settings.readBoolEntry("/shadowedButtonsText", true);
    ...
    /*HACK: hack for gtk-qt: mozillas only works with windows style scrollbars,
            even if the other gtk apps works with other styles, at the moment
            I don't have a way to choose only mozilla apps, because
            qstrcmp( qApp->argv() [ 0 ], "firefox-bin" ) == 0
            does not work (qApp->argv() [ 0 ] is always unknown on gtk-qt apps)
            also, the menu stripe is turned off because it's managed too bad,
            in particular by (of course) firefox
    this hack may be disabled in the future, let's see...*/
    if(  !qstrcmp( qApp->argv() [ 0 ], "unknown" ) )
    {
...
//         widget->setBackgroundMode(PaletteBackground);
        ...
        QSlider *slider = ::qt_cast<QSlider*>(widget);
        if(slider)
        {
...
void PolyesterStyle::unPolish(QWidget* widget)
{
    ...
    else if (::qt_cast<QTabBar*>(widget) || ::qt_cast<QHeader*>(widget)
    ...
    const QSlider *slider = ::qt_cast<QSlider*>(widget);
...
void PolyesterStyle::khtmlWidgetDestroyed(QObject* obj)
{
...
// button area...
    ...
    if( !(_buttonStyle == "gradients" ) /*|| sunken*/ ){
        //polyester style
        renderGradient(p, QRect(drawLeft?r.left()+1:r.left(),
        ...
        const QColor colorTop3 = QColor(colorTop1.rgb()).light(100+_contrast);
        const QColor colorBottom3 = QColor(colorBottom2.rgb()).light(100+_contrast);

        //halfsize is half of height or width depending in the scrollbar is horizontal
        // round on upper int if the size is odd
        int halfsize;
        if( horizontal )
          halfsize = height%2==0?height/2:(height/2)+1;
        else
          halfsize = width%2==0?width/2:(width/2)+1;

        renderGradient(p, QRect(left, top,
                                horizontal?width:width/2, horizontal?height/2:height),
                                colorTop2, colorBottom3, horizontal);
        renderGradient(p, QRect(horizontal?left:left+width/2,
        ...
        renderGradient(p, QRect(drawLeft?r.left()+1:r.left(),
                                drawTop?r.top()+1:r.top(), width, height),
                        buttonColor, buttonColor, horizontal);
    }
// highlighting...
    if(highlightTop) {
...
void PolyesterStyle::renderPixel(QPainter *p,
            const QPoint &pos,
            const int alpha,
            const QColor &color,
                              ...
                              ) const
{
    ...
    if (!flatMode) {
        ...
        renderContour(p, r, g.background(),
                      (isDefault?getColor(g,ButtonContour).dark(100+_contrast*10):getColor(g,ButtonContour)),
                      contourFlags);
        ...
        if(!isDefault)
        {
           renderSurface(p, QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2),
                       g.background(), g.button().light(100+animFrame),
                       (_animateButton?alphaBlendColors( getColor(g,MouseOverHighlight),g.button(), 20*animFrame ):getColor(g,MouseOverHighlight)),
                       _contrast, surfaceFlags);
        }
        else
        {
          //12.8 is the maximum amount of blending (256) / ANIMATIONSTEPS (20)
          QColor buttonColor = alphaBlendColors( g.highlight(), g.button(), (int)(12.8*animFrame)+50);

          renderSurface(p, QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2),
                        g.background(), buttonColor.light(100+animFrame), getColor(g,MouseOverHighlight), _contrast+animFrame, surfaceFlags);
        }
...
void PolyesterStyle::renderMenuBlendPixmap( KPixmap &pix, const QColorGroup &cg,
                            ...
                            bool cornerWidget) const
{
    ...
    // the top part of the tab which is nearly the same for all positions
        ...
        if(!bottom) {
            ...
            if( _highLightTab )
            {
              p->fillRect(Rs.x()+1, Rs.bottom()-1, Rs.width()-2, 2, g.highlight() );
            ...
            renderSurface(p, QRect(Rs.x()+1, Rs.y(), Rs.width()-2, Rs.height()-3), g.background(), g.button(), g.highlight(), _contrast, Is_Horizontal);
    ...
    // some "position specific" paintings...
        ...
        if(!isSingle) {
            ...
            if((!isLast&&!reverseLayout) || (!isFirst&&reverseLayout) ) {
                //for some strange cause i must set the pen color again...
                p->setPen(alphaBlendColors(g.background(), getColor(g, ButtonContour), 50) );
                ...
                renderSurface(p, QRect(r.right(), bottom?r.top()+3:(triangular?r.top()+3:r.top()+4), 1, (triangular?r.height()-6:r.height()-7) ),
                            g.background(), g.button(), getColor(g,MouseOverHighlight), _contrast,
                            Draw_Top|Draw_Bottom|Is_Horizontal/*|((bottom?Highlight_Bottom:Highlight_Top):0)*/);
#endif
            }
        }
    ...
    switch( kpe ) {
        case KPE_SliderGroove: {
            ...
            if (horizontal) {
                ...
                /*renderContour(p, QRect(r.left(), center-3, r.width(), 5),
                              cg.background(), cg.background().dark(enabled?150:130),
                              Draw_Left|Draw_Right|Draw_Top|Draw_Bottom);*/

                /*p->fillRect(r.left()+1, center-1, (int)(coloredWidth), 2,
                            enabled?cg.highlight():cg.background().dark(120));*/
                //colored part
                renderContour(p, QRect(r.left(), center-3, (int)coloredWidth, 5),
                              cg.background(),
                              enabled && _coloredScrollBar?getColor(cg, DragButtonContour, IsEnabled)
                                      :cg.background().dark(130),
                              Draw_Left|Draw_Right|Draw_Top|Draw_Bottom);
                renderGradient(p, QRect(r.left()+1, center-2,
                                        (int)coloredWidth, 3),
                               enabled && _coloredScrollBar?cg.highlight().light(100+_contrast*4)
                                      :cg.background().dark(120-_contrast),
                               enabled && _coloredScrollBar?cg.highlight()
                                      :cg.background().dark(120), true);

                //empty part
                renderContour(p, QRect((int)coloredWidth, center-3,
                                       r.width()-(int)coloredWidth, 5),
                              cg.background(), cg.background().dark(enabled?150:130),
                              Draw_Left|Draw_Right|Draw_Top|Draw_Bottom);
                renderGradient(p, QRect((int)coloredWidth, center-2,
                                        r.width()-(int)coloredWidth-1, 3),
                               cg.background().dark(100+_contrast*4),
                               cg.background().light(100+_contrast), true);
           } else {
                ...
                /*renderContour(p, QRect(center-3, r.top(), 5, r.height()),
                              cg.background(), cg.background().dark(enabled?150:130),
                              Draw_Left|Draw_Right|Draw_Top|Draw_Bottom);*/

                /*p->fillRect(center-1, (int)(coloredHeight), 3, r.height(),
                            enabled?cg.highlight():cg.background().dark(120));*/
                //colored part
                renderContour(p, QRect(center-3, (int)nonColoredHeight,
                                       5, r.height()-(int)nonColoredHeight),
                              cg.background(),
                              enabled && _coloredScrollBar?getColor(cg, DragButtonContour, IsEnabled)
                                      :cg.background().dark(130),
                              Draw_Left|Draw_Right|Draw_Top|Draw_Bottom);
                renderGradient(p, QRect(center-2, (int)nonColoredHeight,
                                        3, r.height()-(int)nonColoredHeight-1),
                               enabled?cg.highlight().light(100+_contrast*4)
                                      :cg.background().dark(120-_contrast),
                               enabled?cg.highlight()
                                      :cg.background().dark(120), false);

                //empty part
                renderContour(p, QRect(center-3, r.top(), 5, (int)nonColoredHeight),
                              cg.background(), cg.background().dark(enabled?150:130),
                              Draw_Left|Draw_Right|Draw_Top|Draw_Bottom);
                renderGradient(p, QRect(center-2, r.y()+1, 3,
                                        (int)nonColoredHeight),
                               cg.background().dark(100+_contrast*4),
                               cg.background().light(100+_contrast), false);
            }
            break;
        }
        ...
        case KPE_SliderHandle: {
                ...
                if (horizontal) {
                    ...
                    p->drawPoint(xcenter+5-4, ycenter+7);
                    // 3d effect of vertex
                    if( enabled && _coloredScrollBar )
                    {
                      if( _lightBorder )
                        p->setPen( cg.highlight().light(110+_contrast*2) );
                      else
                        p->setPen( cg.highlight().dark(110+_contrast*2) );
                    }
                    else
                    {
                      if( _lightBorder )
                        p->setPen( cg.background().light(110+_contrast*2) );
                      else
                        p->setPen( cg.background().dark(110+_contrast*2) );
                    }

                    p->drawPoint(xcenter-5+2, ycenter+4);
                    p->drawPoint(xcenter+5-2, ycenter+4);
                    p->drawPoint(xcenter-5+3, ycenter+5);
                    p->drawPoint(xcenter+5-3, ycenter+5);
                    p->drawPoint(xcenter-5+4, ycenter+6);
                    p->drawPoint(xcenter+5-4, ycenter+6);
                    p->drawPoint(xcenter, ycenter+7);
                    ...
                    mask -= QRegion(xcenter-4, ycenter+4, 2, 4);
                    mask -= QRegion(xcenter-3, ycenter+5, 2, 3);
                    mask -= QRegion(xcenter-2, ycenter+6, 2, 2);
                    mask -= QRegion(xcenter-1, ycenter+7, 2, 1);
                    mask -= QRegion(xcenter+1, ycenter+7, 1, 1);
                    mask -= QRegion(xcenter+1, ycenter+6, 2, 2);
                    mask -= QRegion(xcenter+2, ycenter+5, 2, 3);
                    mask -= QRegion(xcenter+3, ycenter+4, 2, 4);
                    ...
                    p->drawPoint(xcenter+4, ycenter-5+2);
                    p->drawPoint(xcenter+4, ycenter+5-2);
                    p->drawPoint(xcenter+5, ycenter-5+3);
                    p->drawPoint(xcenter+5, ycenter+5-3);
                    p->drawPoint(xcenter+6, ycenter-5+4);
                    p->drawPoint(xcenter+6, ycenter+5-4);
                    p->drawPoint(xcenter+7, ycenter);
                    ...
                    mask -= QRegion(xcenter+4, ycenter-3, 4, 1);
                    mask -= QRegion(xcenter+5, ycenter-2, 3, 1);
                    mask -= QRegion(xcenter+6, ycenter-1, 2, 1);
                    mask -= QRegion(xcenter+7, ycenter,   2, 1);
                    mask -= QRegion(xcenter+6, ycenter+1, 2, 1);
                    mask -= QRegion(xcenter+5, ycenter+2, 3, 1);
                    mask -= QRegion(xcenter+4, ycenter+3, 4, 1);
                    mask -= QRegion(xcenter+4, ycenter+4, 4, 1);
                                ...
                                const QStyleOption &opt ) const
{
    ...
    // -------
          ...
          if(_drawFocusRect) //FIXME: it's necessary to be configurable? naaah :)
            {
              ...
              //also no rouded border when there is no parent widget
              if( r.width() < 0 || r.height() < 0 || (widget && !widget->parentWidget()) ||
                  (widget &&
                      (dynamic_cast<QScrollView*>( widget->parent() ) ||
                       dynamic_cast<QListBox*>(  widget->parent()))
                  ) ||
                  cg.background() == getColor(cg, ButtonContour)
                )
              {
                p->setPen( getColor(cg, ButtonContour) );
                p->drawRect( r );
                break;
              }
              ...
              if( widget )
                backgroundColor = widget->paletteBackgroundColor();
              else
                backgroundColor = cg.background();

              renderContour(p, r, backgroundColor, getColor(cg,ButtonContour),
                            Draw_Left| Draw_Right|Draw_Top|Draw_Bottom|
        ...
        case PE_HeaderSection: {
            ...
            if(!kickerMode) {
                // detect if this is the first/last header item
                bool isFirst = false;
                bool isLast = false;
                // header item id
                int headerId = -1;
                //is the section sorted?
                bool sortedSection = false;
                //orientation
                ...
                QListView *list = NULL;
                if (header) {
                    list = dynamic_cast<QListView*>(header->parent());
                    int headerPos = (header->orientation()==Qt::Horizontal?r.x():r.y());
                    headerId = header->sectionAt( headerPos + header->offset());
                    isFirst = header->mapToIndex(header->sectionAt(headerPos ) +
                                                 header->offset()) == 0;
                    isLast = header->mapToIndex(header->sectionAt(headerPos ) +
                                                header->offset()) == header->count()-1;
                    sortedSection = _colorizeSortedHeader&&(header->sortIndicatorSection() == headerId);
                ...
                else
                {
                    ...
                    if( !list || (list && list->sortColumn() != -1) )
                    {
                       if(on||down)
                           surfaceFlags|=Is_Sunken;
                       else
                       {
                           if( (hoverWidget == p->device()) && headerId == hoverHeaderId )
                           {
                            /*remedy to a Qt bug that triggers only on horizontal headers*/
                            if( (header && header->orientation()==Qt::Vertical) ||
                                (header && (header->sectionPos(headerId) +
                                        header->sectionSize(headerId) -
                                        header->offset() > r.x()))
                               )
                ...
                renderSurface(p, QRect(isFirst?r.left()+1:r.left(), r.top()+1, isFirst?r.width()-2:r.width()-1, r.height()-2),
                                cg.background(),
                  sortedSection?alphaBlendColors(cg.highlight(), cg.button(), 70):cg.button(),
        ...
        case PE_ScrollBarSlider: {
            ...
            uint surfaceFlags = Draw_Left|Draw_Right|Draw_Top|Draw_Bottom;
            if( !_coloredScrollBar && hoverWidget == p->device() )
            {
              if( !horiz )
            ...
            {
                renderSurface(p, QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2),
                        cg.background(), surface, cg.highlight(),
                        _contrast+3, surfaceFlags);
            }
            ...
            if( !horiz && r.height() > 24)
            {
              for( int i = -displacement; i <= displacement; i+=displacement)
              {
                if(_scrollBarLines)
                {
                  ...
                  p->setPen(alphaBlendColors(cg.background(), surface.light(enabled?120:110), 50) );
        ...
        case PE_ScrollBarSubPage: {
            ...
            if (on || down) {
                ...
                if(flags & Style_Horizontal) {
                    ...
                    uint surfaceFlags = Draw_Left|Draw_Right|Draw_Top|Draw_Bottom|Is_Sunken|Is_Horizontal;
                    ...
                    renderContour(p, r, cg.background(), getColor(cg, ButtonContour),
                            contourFlags);
                    QRect gradientRect( xs,y+1,ws,h/2 );
                    renderGradient( p, gradientRect, cg.background().dark(100+_contrast),
                                    cg.background().light(100+_contrast));
                    p->fillRect( QRect(xs,gradientRect.bottom()+1,ws,h-2-gradientRect.height()),
                                 cg.background().light(100+_contrast) );
                ...
                else
                {
                    ...
                    uint contourFlags = Draw_Left|Draw_Right;
                    uint surfaceFlags = Draw_Left|Draw_Right|Draw_Top|Draw_Bottom|Is_Sunken;
                    ...
                    QRect gradientRect( x+1,ys,w/2,hs );
                    renderGradient( p, gradientRect, cg.background().dark(100+_contrast),
                                    cg.background().light(100+_contrast), false);
                    p->fillRect( QRect(gradientRect.right()+1, ys, w-2-gradientRect.width(), hs),
                                 cg.background().light(100+_contrast) );
    ...
    // -----------------
        case PE_ScrollBarSubLine: {
            uint contourFlags = Draw_Left|
                               (r.x()==0||down?Draw_Right:0)|
                                Draw_Top|
                               (r.y()==0||down?Draw_Bottom:0);
          ...
          /*  if(!down && !horiz && r.y() != 0)
            {
                p->setPen(cg.background().light(120));
                p->drawLine(r.left(), r.bottom(), r.right(), r.bottom());
            }*/

            //render two dots at bottom/right corners
            p->setPen(getColor(cg, ButtonContour).light(120));

            if( horiz )
            {
              contourFlags |= Sharp_UpperRight|Sharp_BottomRight;
              if( r.x() != 0 )
                contourFlags |= Sharp_UpperLeft|Sharp_BottomRight;
            }
            else
            {
        ...
        case PE_ScrollBarAddLine: {
            ...
            //render two dots at top/left corners
            p->setPen(getColor(cg, ButtonContour).light(120));
            ...
            renderContour(p, r, cg.background(), getColor(cg, ButtonContour),
                          contourFlags);
            renderSurface(p, QRect(r.left()+(!isNext||!horiz?1:0), r.top()+(!isNext||horiz?1:0), r.width()-(isNext&&horiz?1:2), r.height()-(isNext&&!horiz?1:2)),
                          cg.background(), cg.button(), getColor(cg,MouseOverHighlight), _contrast+3,
                          surfaceFlags);
        ...
        case PE_PanelDockWindow: {
            ...
            if ( _drawToolBarSeparator &&
                ...
                if ( r.width() > r.height() ) {
                    ...
                    {
                      p->setPen( cg.background().dark(100+_contrast*4) );
                      p->drawLine( r.left(), r.bottom(), r.right(), r.bottom() );
                    }
                }
                else {
    ...
    // --------------------------
        ...
        case PE_DockWindowHandle: {
            ...
            int vCenter = r.top()+r.bottom()/2;
            /*handles littler, nicer*/
            if(horiz) {
                for(int j = vCenter-6; j <= vCenter+6; j+=6) {
                    renderDot(p, QPoint(hCenter, j), cg.background(), true, true);
                    counter++;
                }
            } else {
                for(int j = hCenter-12; j <= hCenter+12; j+=6) {
                    renderDot(p, QPoint(j, vCenter), cg.background(), true, true);
                    counter++;
                }
            }
    ...
    // -----------------
        ...
        case PE_ArrowRight: {
            ...
            //move PE_HeaderArrow toghether the headerlabel
            /*HACK: brutally move the header arrow; maybe in the future
                    a more clean solution will be possible*/
            if( pe == PE_HeaderArrow && !QApplication::reverseLayout())
            {
                QHeader *header = dynamic_cast<QHeader*>(p->device() );
                if (header && header->orientation()==Qt::Horizontal)
                {
                    int section = header->sectionAt(x + header->offset());
                    x = header->sectionSize(section) +
                                header->sectionPos(section) -
                                header->offset() - 16;
                }
             }

            a.translate((x+r.width()/2), (r.y()+r.height()/2));
                              ...
                              const QStyleOption& opt) const
{
    ...
    // -----------
        ...
        case CE_ProgressBarContents: {
            ...
            { // Busy indicator
                ...
                renderGradient( p, QRect( r.x(), r.y(), r.width(), r.height()/2 ),
                                bg.dark(100+_contrast), bg );
                p->fillRect( QRect( r.x(), (r.height()/2)+1,
                                    r.width(), r.height()-(r.height()/2)+1 ),
                             bg );
            ...
            else
            {
                ...
                QRect RemptyGradient(reverseLayout?r.left():r.left()+w-1, r.top(),
                                     r.width()-w+1, r.height()/2 );
                QRect Rempty( RemptyGradient.x(), RemptyGradient.bottom()+1,
                              RemptyGradient.width(), r.height()-RemptyGradient.height());
                ...
                renderGradient( p, RemptyGradient, bg.dark(100+_contrast), bg );
   ...
   //--------------
       case CE_TabBarLabel:{
          ...
          const uint text_flags = AlignVCenter | AlignHCenter | ShowPrefix |
                                  DontClip | SingleLine;
          ...
          {
             tr.moveTop( tr.top() -
                         pixelMetric( QStyle::PM_TabBarTabShiftVertical, tb ) );
             tr.moveLeft( tr.left() -
                          pixelMetric( QStyle::PM_TabBarTabShiftHorizontal, tb ) );
          }

          if(_shadowedButtonsText)
          {
             p->setPen( cg.button().dark(120+_contrast));
             p->drawText(QRect(tr.x()+1, tr.y()+1, tr.width(), tr.height()),
                         text_flags, opt.tab()->text());
          }
    ...
    // ----
        case CE_TabBarTab: {
            ...
            if( flags & Style_Selected )
              selected = ThisTab;
            else if( tb->indexOf(tb->currentTab()) - (tb->indexOf(opt.tab()->identifier())) == -1)
              selected = PrevTab;
            else if( tb->indexOf(tb->currentTab()) - (tb->indexOf(opt.tab()->identifier())) == 1)
              selected = NextTab;
...
//                     renderTriangularTab(p, r, cg, (flags & Style_MouseOver), selected, true, pos);
        ...
        case CE_PushButton: {
            ...
            int animFrame = 0;
            if( _animateButton && animWidgets.contains( button )){
                animFrame = animWidgets[button].animFrame;
                g2.setColor( QColorGroup::Button, cg.button().light(100+animFrame) );
            }
        ...
        case CE_PushButtonLabel:
        {
            ...
            int animFrame = 0;
            if( _animateButton && button->isDefault() && animWidgets.contains( button ))
              animFrame = animWidgets[button].animFrame;

            //12.8 is the maximum amount of blending (256) / ANIMATIONSTEPS (20)
            QColor textColor = alphaBlendColors( cg.highlightedText(),
            ...
            if( _shadowedButtonsText && button->isEnabled() && (button->text().isEmpty()) )
            {
              QColor shadowColor = alphaBlendColors( cg.highlight(),
                                                cg.button(),
                                                (int)(12.8*animFrame)
                                              ).dark(120+_contrast);
              ...
              drawItem( p, QRect(x+1, y+1, w, h), AlignCenter|ShowPrefix, button->colorGroup(),
                        ...
                        _useLowerCaseText?button->text().lower():button->text(),
                        -1, &shadowColor );
            }
    ...
    // -----------------------------------------
        case CE_MenuBarItem: {
            ...
            if( _menuBarEmphasis )
            {
                ...
                renderSurface(p, mb->rect(), cg.background(),
            ...
            if (active && focused) {
                if (down) {
                 ...
                 renderContour(p, r, getColor(cg, MenuBarEmphasis),  cg.highlight().dark(100+_contrast*8), Draw_Right|Draw_Top|Draw_Left|Round_UpperRight|Round_UpperLeft);
                    //renderTab( p, QRect(r.left(), r.top(), r.width(), r.height()+2), cg, false, true, false, Single, false, false);
                } else {
                    renderSurface(p, QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2),
                            cg.background(), getColor(cg, MenuBarEmphasis), cg.background(),
            ...
            if ( _shadowedMenuBarText ){
                if (active && focused && down)
                    p->setPen(cg.highlight().dark(100+_contrast*2) );
                else
                    p->setPen(getColor(cg, MenuBarEmphasis).dark(100+_contrast*2) );
            ...
            if (active && focused && down)
                ...
                if( _customMenuBarEmphasisColor )
                    p->setPen( (qGray(_menuBarEmphasisColor.rgb())>150?cg.foreground():white) );
                else
                    p->setPen( cg.foreground() );
    ...
    // ------------------------------------------
        case CE_PopupMenuItem: {
            ...
            int stripeWidth = max(checkcol+(int)(checkcol/8),22);
              ...
              if( _menuStripe )
              {
                  if( !QApplication::reverseLayout() )
                    p->fillRect( QRect(r.left(), r.top(), stripeWidth, r.height()),
                                  cg.background().dark( 100+_contrast ) );
                  else
                    p->fillRect( QRect(r.right()-23, r.top(), stripeWidth, r.height()),
                                  cg.background().dark( 100+_contrast ) );
              }
            }
            //...or draw the transparency pixmap
            else
            {
                p->drawPixmap( r.topLeft(), *widget->erasePixmap(), r );
                if( _menuStripe )
                {
                  //do this loop only ONE time :)
                  //this could give problems to apps which have popup menus with different icon sizes...
                  if( alphaStripe->isNull() )
                  {
                    alphaStripe->create(stripeWidth, r.height(), stripeWidth);
                    alphaStripe->setAlphaBuffer(true);
                    for( int i = 0; i < stripeWidth; i++)
                      for( int j = 0; j < r.height(); j++)
                        alphaStripe->setPixel(i,j, qRgba(0, 0 , 0, 20));
                  }

                  if( !QApplication::reverseLayout() )
                    p->drawPixmap( r.topLeft(), *alphaStripe );
                  else
                    p->drawPixmap( r.topRight(), *alphaStripe );
                }
            }

            // is this menu item highlighted?
            if (active)
            {
                QRect r2 =  QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2);
                QRect r3 =  QRect(r.left()+2, r.top()+2, r.width()-4, r.height()-4);

                if (enabled)
                {
                    if (_buttonMenuItem  )
                    {
                        renderSurface(p, r3, cg.background(), cg.highlight(), cg.highlight(),
                                _contrast+3, Is_Horizontal|Draw_Right|Draw_Top|Draw_Bottom|Draw_Left);
                        renderContour(p, r2, cg.background(),  cg.highlight().dark(100+_contrast*8), Draw_Right|Draw_Top|Draw_Bottom|Draw_Left|Round_UpperRight|Round_UpperLeft|Round_BottomRight|Round_BottomLeft|Draw_AlphaBlend);
                    }
                    else
                ...
                //disabled
                    ...
                    if (_buttonMenuItem  )
                    {
                        renderSurface(p, r3, cg.background(), cg.background().light(120), cg.highlight(),
                                _contrast+3, Is_Horizontal|Round_UpperLeft|Round_BottomRight|Round_BottomLeft);
                        renderContour(p, r2, cg.background(),  cg.background().dark(100+_contrast*5), Draw_Right|Draw_Top|Draw_Bottom|Draw_Left|Round_UpperRight|Round_UpperLeft|Round_BottomRight|Round_BottomLeft|Draw_AlphaBlend);
                    }
                    else
                       renderSurface(p, r, cg.background(), cg.background().light(120), cg.highlight(),
                               _contrast+3, Is_Horizontal|Draw_Top|Draw_Bottom|Draw_Left|Draw_Right);
                }
            }
            ...
            if ( mi->iconSet() )
            {
                ...
                if ( checkable && /*!active &&*/ mi->isChecked() )
                   drawPrimitive(QStyle::PE_Indicator, p, cr, cg,
                                 QStyle::Style_Enabled | QStyle::Style_Active |  QStyle::Style_Off );
            ...
            {
             int style = QStyle::Style_Enabled | QStyle::Style_Active |  QStyle::Style_On ;
             drawPrimitive(QStyle::PE_Indicator, p, cr, cg, style);
            }
    ...
    //
        ...
        case CE_MenuBarEmptyArea:
            if( _menuBarEmphasis )
            {
              renderSurface(p, r, cg.background(),
                            getColor(cg, MenuBarEmphasis),
                            cg.background(),
                            _contrast+3, Is_Horizontal);

              if( _menuBarEmphasisBorder )
                renderContour(p, r, cg.background(), getColor(cg, MenuBarEmphasis).dark(115),
...
//             }
        ...
        case CE_HeaderLabel:
        {
            ...
            QString s = header->label(headerId);
            ...
            if( !s.isNull() )
            {
               ...
               p->drawText( r.x()<=2&&!reverse?2:r.x(),
                                 r.y(), r.width(), r.height(),
                                 text_flags, s, t );
            }
            QIconSet *iS = header->iconSet(headerId);
            if( iS )
            {
              QIconSet::Mode  mode  = QIconSet::Normal;
              QIconSet::State state = QIconSet::On;

              QPixmap pixmap = iS->pixmap( QIconSet::Small, mode, state );
                                     ...
                                     const QStyleOption& opt) const
{
    ...
    // --------
        case CC_ComboBox: {
            ...
            const QColor buttonColor = enabled?cg.button().light(100+animFrame):cg.background();
            const QColor highlightColor = (_animateButton?alphaBlendColors( getColor(cg,MouseOverHighlight),cg.button(), 20*animFrame ):getColor(cg,MouseOverHighlight));
            ...
            if ( (animFrame != 0) || (widget == hoverWidget) || (flags & Style_MouseOver)) {
                surfaceFlags |= Is_Highlight;
                ...
                if (hasFocus) {
                    drawPrimitive(PE_FocusRect, p,
                        QRect(RcontentSurface.x() + 2,
                            ...
                            RcontentSurface.height() - 4), cg, Style_Default, opt);
                }
            } else {
    ...
    // ----------
        case CC_ToolButton: {
            ...
            SFlags bflags = flags,
                    mflags = flags;
            SFlags borderFlags = Draw_Left|Draw_Right|Draw_Top|Draw_Bottom|
                    Round_UpperLeft|Round_UpperRight|Round_BottomLeft|Round_BottomRight;
            ...
            if (kornMode) {
                ...
                p->fillRect(r, cg.background().light(100+animFrame));
    ...
    // -----------
        case CC_SpinWidget: {
            ...
            QSpinWidget *sw = (QSpinWidget *)(widget);// **hack**: cast away const-ness for animation support!
            SFlags sflags = flags;
            PrimitiveElement pe;
            ...
            if ( (animFrame != 0) || (widget == hoverWidget) || (sflags & Style_MouseOver)) {
                surfaceFlags |= Is_Highlight;
                surfaceFlags |= Highlight_Top|Highlight_Left|Highlight_Right;
            }
            if (active==SC_SpinWidgetUp) surfaceFlags|=Is_Sunken;
            if(!enabled) surfaceFlags|=Is_Disabled;
            renderSurface(p, upRect, cg.background(), buttonColor, highlightColor,
                           _contrast, surfaceFlags);
            surfaceFlags = Draw_Left|Draw_Right|Draw_Top|Draw_Bottom|Is_Horizontal;
            if(reverseLayout) {
            ...
            if(!enabled) surfaceFlags|=Is_Disabled;
            renderSurface(p, downRect, cg.background(), buttonColor, highlightColor,
                           _contrast, surfaceFlags);
            ...
            if (_inputFocusHighlight && hasFocus && enabled)
            {
              p->setPen( getColor(cg,FocusHighlight).dark(100+(_lightBorder?-30:30)) );
            }
            else
            {
              p->setPen(inputColor.dark(100+(_lightBorder?-30:30)) );
...
QRect PolyesterStyle::subRect(SubRect r, const QWidget *widget) const
{
    switch (r) {
...
int PolyesterStyle::pixelMetric(PixelMetric m, const QWidget *widget) const
{
    switch(m) {
    // TABS
    // ----
...
// #endif
    ...
    // ------
        ...
        case PM_MenuBarFrameWidth:
            return 1; //FIXME: change menubar height
...
int PolyesterStyle::kPixelMetric(KStylePixelMetric kpm, const QWidget *widget) const
{
    switch(kpm)
    {
                                   ...
                                   QStyleHintReturn* returnData ) const
{
    switch (stylehint) {
        ...
        case QStyle::SH_TabBar_Alignment:
            /*if reverseLayout is enabled, the centering of tabs
              is disabled, due to a bug in rendering*/
            if (_centeredTabBar && !QApplication::reverseLayout())
             return Qt::AlignHCenter;

        //Suggestion by Daniel Molkentin,
        //algorithm from QT4 QWindowsXPStyle Copyright 2006 Trolltech
//this exists since Qt3.2
...
bool PolyesterStyle::eventFilter(QObject *obj, QEvent *ev)
{
    ...
    //header mouse over
    if ( ::qt_cast<QHeader*>(obj) ) {
        QWidget* widget = static_cast<QWidget*>(obj);
        if ((ev->type() == QEvent::Leave) && static_cast<QWidget*>(obj)->isEnabled())
        {
           hoverWidget = 0;
           hoverHeaderId = -1;
           widget->repaint(false);
        ...
        else if ((ev->type() == QEvent::MouseMove) && static_cast<QWidget*>(obj)->isEnabled())
        {
           ...
           QHeader* header = static_cast<QHeader*>(widget);

           hoverWidget = widget;
           int tempHover = hoverHeaderId;

           if( header->orientation()==Qt::Horizontal )
            hoverHeaderId = header->sectionAt( mEvent->x()+header->offset() );
           else
            hoverHeaderId = header->sectionAt( mEvent->y()+header->offset() );

           if( tempHover != hoverHeaderId )
             widget->update();
        }
    }
            ...
            ::qt_cast<QRadioButton*>(obj) || ::qt_cast<QToolButton*>(obj) ||  obj->inherits("QSplitterHandle") )
    {
        if ((ev->type() == QEvent::Enter) && static_cast<QWidget*>(obj)->isEnabled())
        {
            ...
            if(_animateButton && !::qt_cast<QRadioButton*>(obj)
               && !::qt_cast<QCheckBox*>(obj)){
...
QColor PolyesterStyle::getColor(const QColorGroup &cg, const ColorType t, const bool enabled)const
{
    return getColor(cg, t, enabled?IsEnabled:IsDisabled);
}

QColor PolyesterStyle::getColor(const QColorGroup &cg, const ColorType t, const WidgetState s)const
{
    ...
    switch(t) {
        ...
        case DragButtonContour: {
            if(enabled) {
                if(pressed)
                    return _coloredScrollBar?cg.highlight().dark(130+_contrast*6)
                                            :cg.background().dark(130+_contrast*6); // bright
                else if(highlighted)
                    return _coloredScrollBar?cg.highlight().dark(130+_contrast*9)
                                            :cg.background().dark(130+_contrast*9); // dark
                else
                    return _coloredScrollBar?cg.highlight().dark(130+_contrast*8)
                                            :cg.background().dark(130+_contrast*8); // normal
            } else {
        ...
        case DragButtonSurface: {
            if(enabled) {
                if(pressed)
                    return _coloredScrollBar?cg.highlight().dark(100-_contrast):cg.button().dark(100-_contrast);  // bright
                else if(highlighted)
                    return _coloredScrollBar?cg.highlight().light(100+_contrast):cg.button().light(100+_contrast); // dark
                else
                    return _coloredScrollBar?cg.highlight():cg.button();                      // normal
            } else {
